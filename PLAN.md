...
**What:** Add UI translations with language switching. Initially 6 languages: English (en), Russian (ru), Greek (el), German (de), French (fr), Spanish (es). Later expanded to 9 languages (added zh, ar, sr in Phase 9).
...
## Phase 9: Platform Expansion (Steps 36-44) — DONE

Additional features, connectors, and platform hardening completed after Phase 8.

| Step | Feature | Status | Notes |
|------|---------|--------|-------|
| 36 | Data Catalog | DONE | `CatalogEntry` model, `catalog_service.py`, browse UI with model detail pages |
| 37 | SQL Editor | DONE | Full-screen editor page with autocomplete, compiled SQL preview |
| 38 | File Uploads | DONE | `UploadedFile` model, `file_upload_service.py`, drag-and-drop CSV/JSON/Parquet |
| 39 | Database Backups | DONE | `backup_service.py`, `backup_state.py`, metadata backup/restore |
| 40 | Additional Locales | DONE | Added `zh.json` (Chinese), `ar.json` (Arabic), `sr.json` (Serbian) — now 9 locales |
| 41 | Google Sheets Source | DONE | `google_sheets_source.py`, dedicated dlt source adapter |
| 42 | MongoDB Source | DONE | `mongodb_source.py`, dedicated dlt source adapter |
| 43 | ClickHouse Destination | DONE | Added to dlt destination factory |
| 44 | Dependency Checks | DONE | `dependency_check.py`, pre-delete validation to prevent orphaned references |

### Hooks System (Step 45) — DONE

Generic event bus (`hooks.py`) for plugin extensibility. Core services emit 6 events:

- `connection.before_create` — pre-creation quota check
- `schedule.before_create` — pre-creation quota check
- `membership.before_create` — pre-creation seat limit check
- `run.upload_completed` — post-upload metering
- `run.models_completed` — post-pipeline metering
- `run.transformation_completed` — post-transformation metering

Used by `datanika-cloud` plugin for billing quota enforcement and usage metering.
