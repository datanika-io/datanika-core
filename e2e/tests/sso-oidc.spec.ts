...
test.describe("SSO OIDC: Authentik @slow", () => {
  ...
  test("OIDC login redirects to Authentik authorization endpoint", async ({ page }) => {
    ...
    // Authentik wraps the authorize URL inside its login flow:
    // /if/flow/default-authentication-flow/?next=%2Fapplication%2Fo%2Fauthorize%2F...
    const decoded = decodeURIComponent(finalUrl);
    expect(decoded).toContain("/application/o/authorize/");
    expect(decoded).toContain("client_id=datanika-oidc-e2e");
  });
  ...
  test("OIDC full flow: login via Authentik → session created", async ({ page }) => {
    ...
    // 2. Authentik login flow — may be /if/flow/... or /application/o/authorize/
    await page.waitForURL(/localhost:9000/, { timeout: 10000 });

    // Authentik identification stage: enter username/email
    await page.locator('input[name="uidField"]').fill(SSO_USER_EMAIL);
    await page.locator('button[type="submit"], .pf-c-button--primary, ak-stage-identification button').first().click();
