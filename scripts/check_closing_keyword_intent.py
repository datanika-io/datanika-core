...
def render(findings_: list[Finding], where: str) -> str:
    ...
    head += [
        ...
        "",
        "  🚨 Do NOT escape the hash. An HTML entity for `#` contains a hash followed by digits,",
        "     so escaping does not remove a reference -- it creates a different one, pointing at",
        "     whatever issue those digits name. This check folds those forms back before reading.",
        "",
