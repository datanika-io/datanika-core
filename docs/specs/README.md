...
| [`SPEC_USAGE_VISIBILITY.md`](SPEC_USAGE_VISIBILITY.md) | We bill in a unit no screen displays. One contract over three surfaces — the dashboard card ([#1513](https://github.com/datanika-io/datanika-core/issues/1513)), the billing usage card (`cloud#180`) and the upgrade card (`cloud#174`) — so they cannot disagree. 🚨 §1b: **there are TWO gates, not one, and removing the feature flag alone renders nothing** — `usage.get_summary` returns `bytes_limit=0` for a Free org whose plan row carries 10 GiB, measured against a running stack with a fabricated-org control. 🔑 §1c: `fill_usage_summary`'s own docstring records this defect being fixed **for runs**, while it is live for bytes. | Contract, decided 2026-09-23. Not implemented: core gates on a UX flag and cloud never assigns the byte fields, read 2026-09-23 |
