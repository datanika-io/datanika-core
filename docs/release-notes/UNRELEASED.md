...
None recorded since `v0.3.0`. Add a section here for anything that behaves differently for a
self-hoster after the next tag, in the shape `v0.2.0.md` uses: who this affects, what changed, and
what to do. The file is rotated to `<tag>.md` when that tag is cut.
