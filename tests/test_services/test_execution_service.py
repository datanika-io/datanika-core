...
class TestCreateRun:
    def test_creates_pending_run(self, svc, db_session, org, pipeline):
        run = svc.create_run(db_session, org.id, NodeType.PIPELINE, pipeline.id)
        assert isinstance(run, Run)
        assert isinstance(run.id, int)
        assert run.status == RunStatus.PENDING
        assert run.org_id == org.id
        assert run.target_type == NodeType.PIPELINE
        assert run.target_id == pipeline.id
        assert run.started_at is None
        assert run.finished_at is None


class TestStartRun:
    def test_sets_running_and_started_at(self, svc, db_session, org, pipeline):
        run = svc.create_run(db_session, org.id, NodeType.PIPELINE, pipeline.id)
...
class TestCompleteRun:
    def test_sets_success_and_finished_at(self, svc, db_session, org, pipeline):
        run = svc.create_run(db_session, org.id, NodeType.PIPELINE, pipeline.id)
...
class TestFailRun:
    def test_sets_failed_and_finished_at(self, svc, db_session, org, pipeline):
        run = svc.create_run(db_session, org.id, NodeType.PIPELINE, pipeline.id)
        svc.start_run(db_session, run.id)
...
class TestCancelRun:
    def test_cancel_pending(self, svc, db_session, org, pipeline):
        run = svc.create_run(db_session, org.id, NodeType.PIPELINE, pipeline.id)
        cancelled = svc.cancel_run(db_session, run.id)
        assert cancelled is not None
        assert cancelled.status == RunStatus.CANCELLED

    def test_cancel_running(self, svc, db_session, org, pipeline):
        run = svc.create_run(db_session, org.id, NodeType.PIPELINE, pipeline.id)
        svc.start_run(db_session, run.id)
        cancelled = svc.cancel_run(db_session, run.id)
        assert cancelled is not None
    ...
    def test_cancel_completed_fails(self, svc, db_session, org, pipeline):
        run = svc.create_run(db_session, org.id, NodeType.PIPELINE, pipeline.id)
        svc.start_run(db_session, run.id)
...
class TestListRuns:
    ...
    def test_multiple(self, svc, db_session, org, pipeline):
        svc.create_run(db_session, org.id, NodeType.PIPELINE, pipeline.id)
        svc.create_run(db_session, org.id, NodeType.PIPELINE, pipeline.id)
        assert len(svc.list_runs(db_session, org.id)) == 2

    def test_filter_by_status(self, svc, db_session, org, pipeline):
        r1 = svc.create_run(db_session, org.id, NodeType.PIPELINE, pipeline.id)
        svc.create_run(db_session, org.id, NodeType.PIPELINE, pipeline.id)
    ...
    def test_filter_by_target(self, svc, db_session, org, pipeline):
        svc.create_run(db_session, org.id, NodeType.PIPELINE, pipeline.id)
    ...
    def test_limit(self, svc, db_session, org, pipeline):
        for _ in range(5):
            svc.create_run(db_session, org.id, NodeType.PIPELINE, pipeline.id)
    ...
    def test_filters_by_org(self, svc, db_session, org, other_org, pipeline):
        svc.create_run(db_session, org.id, NodeType.PIPELINE, pipeline.id)
        svc.create_run(db_session, other_org.id, NodeType.PIPELINE, 1)
        result = svc.list_runs(db_session, org.id)
        assert len(result) == 1
