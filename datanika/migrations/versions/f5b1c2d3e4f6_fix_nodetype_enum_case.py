...
def upgrade() -> None:
    ...
    op.execute(
        "UPDATE catalog_entries SET origin_type = 'UPLOAD' WHERE origin_type = 'upload'"
    )
    op.execute(
    ...
    op.execute(
        "UPDATE catalog_entries SET origin_type = 'PIPELINE' WHERE origin_type = 'pipeline'"
    )
    ...
    op.execute(
        "UPDATE schedules SET target_type = 'UPLOAD' WHERE target_type = 'PIPELINE'"
    )
    op.execute(
        "UPDATE runs SET target_type = 'UPLOAD' WHERE target_type = 'PIPELINE'"
    )

    # dependencies: fix any lowercase values (may not exist yet)
    for col in ("upstream_type", "downstream_type"):
        op.execute(
            f"UPDATE dependencies SET {col} = 'UPLOAD' WHERE {col} = 'upload'"
        )
        op.execute(
            f"UPDATE dependencies SET {col} = 'TRANSFORMATION'"
            f" WHERE {col} = 'transformation'"
        )
        op.execute(
            f"UPDATE dependencies SET {col} = 'PIPELINE' WHERE {col} = 'pipeline'"
        )
...
def downgrade() -> None:
    ...
    op.execute(
        "UPDATE schedules SET target_type = 'PIPELINE' WHERE target_type = 'UPLOAD'"
    )
    op.execute(
        "UPDATE runs SET target_type = 'PIPELINE' WHERE target_type = 'UPLOAD'"
    )
