...
# and constructing a source RESOLVES THE HOST — see `no_live_dns` in `conftest.py`
# (core#1280, moved out of this module by core#1597).
...
def _stub_dns(no_live_dns):
    """Every test here builds a source, and building one RESOLVES THE HOST (core#1280).

    The fixture body used to live in this module. core#1597 moved it to ``conftest.py``,
    unchanged in behaviour, because a **per-module** remedy is why the same defect was
    still open in three other modules: the fixture cannot be imported across test
    modules (``tests/test_fixture_sharing.py``), so the only ways to reuse it were to
    copy it or to move it. The controls that keep it a network stub rather than a guard
    bypass moved with it, to
    ``tests/test_services/test_no_live_dns_is_a_stub_not_a_bypass.py`` — they belong
    beside the fixture, not beside one of its four callers.
    """
    return no_live_dns
